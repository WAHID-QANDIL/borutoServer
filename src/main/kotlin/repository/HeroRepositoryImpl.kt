...
const val NEXT_PAGE_NUMBER = "nextPage"
const val PREVIOUS_PAGE_NUMBER = "prevPage"
...
class HeroRepositoryImpl : HeroRepository {
    ...
    private fun calculatePageNumber(pageNumber: Int):Pages
    {
    ...
    private val statusOK = HttpStatusCode.OK.toString()
    override suspend fun getAllHeroes(pageNumber: Int): ApiResponse {
        return ApiResponse(
            success = true,
            message = statusOK,
            nextPage = calculatePageNumber(pageNumber).nextPage,
            prevPage = calculatePageNumber(pageNumber).prevPage,
            heroes = heroes[pageNumber]!!,
    ...
    override suspend fun searchHeroes(name: String?): ApiResponse {
        return ApiResponse(
            success = true,
            message = statusOK,
            heroes = getMatchedHeroesByName(name)
        )
    }

    private fun getMatchedHeroesByName(query:String?):List<Hero>
    {
        val matchedHeroesList = mutableListOf<Hero>()
        if (!query.isNullOrEmpty())
        heroes.forEach{ (_, hero) ->
            hero.forEach {
                if (it.name.lowercase().contains(query.lowercase())){
                    matchedHeroesList.add(it)
                }
            }
        }
        return matchedHeroesList
    }
